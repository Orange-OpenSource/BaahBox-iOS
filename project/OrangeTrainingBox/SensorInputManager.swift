...
class SensorInputManager {
    ...
    public func analyseCompressedRawInput(_ rawInput: [UInt8]) {
       ...
       let rawM1 = bytesToValue(coeff: rawInput[0], add: rawInput[1])
       let rawM2 = bytesToValue(coeff: rawInput[2], add: rawInput[3])
        musclesInput.muscle1 = calibrateAnalogInput(rawM1)
        musclesInput.muscle2 = calibrateAnalogInput(rawM2)
